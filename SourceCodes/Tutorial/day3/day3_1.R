...
census <- read.csv("101_DT_1IN1002_F_2010.csv", skip=2)
head(census)
# 행정구역 경계 파일

file.exists('CTPRVN_201703/TL_SCCO_CTPRVN.shp')
# 1. 시도단위
s <- shapefile("CTPRVN_201703/TL_SCCO_CTPRVN.shp")  #raster
hrr.shp<-readShapePoly("CTPRVN_201703/TL_SCCO_CTPRVN.shp", verbose=TRUE)
plot(hrr.shp) # plot shape file in R  

dbf <- read.dbf("CTPRVN_201703/TL_SCCO_CTPRVN.dbf")
...
writePolyShape(jeju1.shp, "jeju1.shp")

# 2. 시군구단위
sig.shp<-readShapePoly("SIG_201703/TL_SCCO_SIG.shp", verbose=TRUE)
plot(sig.shp)
...
writePolyShape(jeju2.shp, "jeju2.shp")

# 3. 읍면동단위
eup.shp<-readShapePoly("EMD_201703/TL_SCCO_EMD.shp", verbose=TRUE)
plot(eup.shp)
...
writePolyShape(jeju3.shp, "jeju3.shp")

rm(list=ls())  #모든 저장된 자료 제거

# 저장한 자료 가져오기
jeju1<- readShapePoly("jeju1.shp", verbose= TRUE)
jeju2<- readShapePoly("jeju2.shp", verbose= TRUE)
jeju3<- readShapePoly("jeju3.shp", verbose= TRUE)
